...
Again, a couple ways of looking at this inferentially.  First we can do a Kolmogorov–Smirnov  to get a p-value about these being drawn from the same population distribution.  We can also do a two-sample t test to look for differences in means.  Finally, we can again fit a couple linear models and use information theory. 
...
## Revised Probability-based assignments

First we need to get th distributional parameters of the known-orgins birds by age class.
...
s2f_plot <- ggmap(base, darken = c(.3,"#FFFFFF")) +
  ...
  geom_text(aes(x = -105.05, y = 39.17, label = "MG")) +
  geom_text(aes(x = -105.05, y = 39.11, label = "HG")) +
  geom_text(aes(x = -105.21, y = 39.175, label = "HF")) +
  geom_text(aes(x = -105.12, y = 39.055, label = "TC")) +
...
Make function to calculate straight-line distance from X,Y coords (pythagorean theorem and such...)
